...
from fbx import KFbxDeformer
from fbx import KFbxXMatrix
...
def DisplayLink(pGeometry):
    ...
    for i in range(lSkinCount):
        lClusterCount = pGeometry.GetDeformer(i, KFbxDeformer.eSKIN).GetClusterCount()
        for j in range(lClusterCount):
            DisplayInt("    Cluster ", i)

            lCluster = pGeometry.GetDeformer(i, KFbxDeformer.eSKIN).GetCluster(j)
