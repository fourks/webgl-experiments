...
#from fbx import KFbxAnimStack
#from fbx import KFbxAnimLayer
#from fbx import KFbxProperty
#from fbx import KFbxNodeAttribute

def DisplayAnimation(pScene):
    for i in range(pScene.GetSrcObjectCount(KFbxAnimStack.ClassId)):
        lAnimStack = pScene.GetSrcObject(KFbxAnimStack.ClassId, i)
...
def DisplayAnimationStack(pAnimStack, pNode, isSwitcher):
    nbAnimLayers = pAnimStack.GetSrcObjectCount(KFbxAnimLayer.ClassId)
    ...
    for l in range(nbAnimLayers):
        lAnimLayer = pAnimStack.GetSrcObject(KFbxAnimLayer.ClassId, l)
...
def DisplayChannels(pNode, pAnimLayer, DisplayCurve, DisplayListCurve, isSwitcher):
    ...
    if lNodeAttribute:
        ...
        if light:
            ...
            lAnimCurve = light.ConeAngle.GetCurve(pAnimLayer)
            if lAnimCurve:
                print("        Cone Angle")
                DisplayCurve(lAnimCurve)
        ...
        if lNodeAttribute.GetAttributeType() == KFbxNodeAttribute.eMESH or \
            lNodeAttribute.GetAttributeType() == KFbxNodeAttribute.eNURB or \
            lNodeAttribute.GetAttributeType() == KFbxNodeAttribute.ePATCH:
            lGeometry = lNodeAttribute

            lBlendShapeDeformerCount = lGeometry.GetDeformerCount(KFbxDeformer.eBLENDSHAPE)
            for lBlendShapeIndex in range(lBlendShapeDeformerCount):
                lBlendShape = lGeometry.GetDeformer(lBlendShapeIndex, KFbxDeformer.eBLENDSHAPE)
    ...
    while lProperty.IsValid():
        if lProperty.GetFlag(KFbxProperty.eUSER):
            ...
            if lDataType.GetType() == eBOOL1 or lDataType.GetType() == eDOUBLE1 or lDataType.GetType() == eFLOAT1 or lDataType.GetType() == eINTEGER1:
                lMessage =  "        Property "
                ...
                for c in range(lCurveNode.GetCurveCount(0)):
                    ...
                    if lAnimCurve:
                        DisplayCurve(lAnimCurve)
            elif lDataType.GetType() == eDOUBLE3 or lDataType.GetType() == eDOUBLE4 or lDataType.Is(DTColor3) or lDataType.Is(DTColor4):
                if lDataType.Is(DTColor3) or lDataType.Is(DTColor4):
